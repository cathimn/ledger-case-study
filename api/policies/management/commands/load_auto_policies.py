...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(csv_file, 'r') as file:
            ...
            for row in reader:
                AutoPolicy.objects.create(
                    ...
                    driver_age=row[2],
                    driver_gender=row[3],
                    driver_employment=row[4],
                    driver_marital=row[5],
                    driver_location=row[6],
                    vehicle_age=row[7],
                    vehicle_model=row[8],
        ...
        self.stdout.write(
            self.style.SUCCESS('Successfully loaded {} auto policies'.format(AutoPolicy.objects.all().count())))
